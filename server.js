...
<meta name="apple-mobile-web-app-title" content="WH Engine"/>
<title>WritersHubUS Content Engine</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#001529;font-family:'Segoe UI',system-ui,sans-serif;color:#fff}
@keyframes spin{to{transform:rotate(360deg)}}
button{cursor:pointer;transition:all .18s;font-family:inherit}
button:not(:disabled):hover{opacity:.88}
pre{white-space:pre-wrap;font-family:inherit}
a{text-decoration:none}
video{width:100%;border-radius:9px;display:block}
input{font-family:inherit;color:#fff;outline:none;box-sizing:border-box}
</style>
</head>
<body>
<div id="root"><div style="display:flex;align-items:center;justify-content:center;height:100vh;color:#FFD700;font-size:14px">Loading WritersHub Engine...</div></div>
...
function Spin({size=12,color="#fff"}){
  return <span style={{width:size,height:size,border:`2px solid ${color}25`,borderTopColor:color,borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/>;
}
...
function App(){
  ...
  useEffect(()=>{
    try{
      const h=store.get("wh_hist"); if(h) setHistory(JSON.parse(h));
      const s=store.get("wh_proxy"); if(s){setProxyUrl(s);setProxyInput(s);}
      const k=store.get("wh_xai"); if(k) setXaiKey(k);
    }catch(_){}
  ...
  const switchTab=t=>{setTab(t);setFmt(0);setOutput("");setGrokShortOut("");setGrokOut("");setVideoUrl("");setVideoStatus("");};

  const saveConfig=async()=>{
    const s=proxyInput.trim().replace(/\/$/,""),k=keyInput.trim();
    setProxyUrl(s);if(k)setXaiKey(k);
    store.set("wh_proxy",s);if(k)store.set("wh_xai",k);
    setProxyOk(null);
    try{const r=await fetch(`${s}/`);setProxyOk(r.ok);showT(r.ok?"✅ Proxy connected!":"❌ Proxy error");}
    catch(_){setProxyOk(false);showT("❌ Cannot reach proxy");}
    setShowConfig(false);
  };

  const callClaude=async(sys,msg,max=1000)=>{
    const r=await fetch(`${DEFAULT_PROXY}/api/claude`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:max,system:sys,messages:[{role:"user",content:msg}]})});
  ...
  const generate=async()=>{
    ...
    const used=history.filter(h=>h.tab===tab).slice(-10).map(h=>h.hook).filter(Boolean);
    const avoid=used.length?`\n\nSTRICTLY AVOID these hooks:\n${used.map((h,i)=>`${i+1}. "${h}"`).join("\n")}`:"";
    const ps={
      reel:`Create a "${f}" Reel script for WritersHubUS with the "${a.label}" angle. Timed: [VISUAL], (VO), ON-SCREEN TEXT, timestamps. Start: HOOK: [line]\n${avoid}`,
      post:`Create a "${f}" IG Post for WritersHubUS "${a.label}" angle. Human, direct. Start: HOOK: [line]\n${avoid}`,
      podcast:`Create "${f}" for WritersHubUS podcast. Angle: "${a.label}". Coach-like. Start: HOOK: [line]\n${avoid}`,
      street:`Create "${f}" street video for WritersHubUS. Raw cinematic. Angle: "${a.label}". [SCENE],(VO), text. Start: HOOK: [line]\n${avoid}`,
    };
    try{
      ...
      setHistory(nh);store.set("wh_hist",JSON.stringify(nh));
  ...
  const genGrok=async short=>{
    ...
    const p=short
      ?`Convert to Grok SHORT prompt — 10-15s only. One scene, one message.\n\nScript:\n${output}\n\nBrand: WritersHubUS. Navy, gold, white. Athletic-academic. Vertical 9:16.`
      :`Convert to full Grok video prompt. Scene breakdown, camera, lighting, text overlays.\n\nScript:\n${output}\n\nBrand: WritersHubUS. Navy, gold, white. Cinematic. Vertical 9:16.`;
    try{const text=await callClaude(GROK_SYS,p,700);short?setGrokShortOut(text):setGrokOut(text);}
  ...
  const generateVideo=async(prompt,dur)=>{
    ...
    setVideoLoading(true);setVideoUrl("");setVideoStatus("Submitting to Grok...");setActivePrompt(prompt);
    const hdrs={"Content-Type":"application/json"};if(xaiKey)hdrs["x-xai-key"]=xaiKey;
    try{
      ...
      const{requestId}=d;
      setVideoStatus("Generating — 1-3 minutes...");
      let n=0;
      pollRef.current=setInterval(async()=>{
        n++;if(n>72){clearInterval(pollRef.current);setVideoStatus("Timed out.");setVideoLoading(false);return;}
        try{
          ...
          else if(pd.status==="expired"){clearInterval(pollRef.current);setVideoStatus("Expired.");setVideoLoading(false);}
          else setVideoStatus(`Generating... (${n*5}s)`);
        }catch(e){setVideoStatus(`Poll error: ${e.message}`);}
      },5000);
    }catch(_){setVideoStatus("Cannot reach proxy.");setVideoLoading(false);}
  };
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#001529",minHeight:"100vh",color:"#fff"}}>
      {toast&&<div style={{position:"fixed",top:14,left:"50%",transform:"translateX(-50%)",background:"#00A86B",color:"#fff",padding:"9px 22px",borderRadius:8,fontSize:12,fontWeight:700,zIndex:999,whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(0,0,0,.5)"}}>{toast}</div>}
      ...
      <div style={{background:"linear-gradient(135deg,#001F3F,#002D6B)",padding:"14px 16px 12px",borderBottom:"2px solid #FFD700",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:6,alignItems:"center"}}>
          {history.length>0&&<div style={{background:"rgba(255,215,0,.12)",border:"1px solid rgba(255,215,0,.3)",borderRadius:20,padding:"3px 9px",fontSize:10,color:"#FFD700",fontWeight:700}}>{history.length} made</div>}
          ...
          {history.length>0&&<button onClick={()=>{setHistory([]);store.del("wh_hist");showT("Reset!");}} style={{background:"none",border:"none",color:"rgba(255,80,80,.5)",fontSize:10,fontWeight:700,padding:0}}>Reset</button>}
        </div>
      </div>

      {showConfig&&(
        <div style={{background:"rgba(109,40,217,.1)",borderBottom:"1px solid rgba(139,92,246,.3)",padding:"14px 16px"}}>
          <div style={{fontSize:11,color:"#A78BFA",fontWeight:700,marginBottom:10}}>⚡ Proxy Configuration</div>
          <div style={{marginBottom:8}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:4}}>Proxy URL</div>
            <input value={proxyInput} onChange={e=>setProxyInput(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,.07)",border:"1px solid rgba(139,92,246,.4)",borderRadius:7,fontSize:12,padding:"8px 12px"}}/>
          </div>
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:4}}>xAI API Key</div>
            <input type="password" value={keyInput} onChange={e=>setKeyInput(e.target.value)} placeholder={xaiKey?"Key saved — enter to update":"xai-..."} style={{width:"100%",background:"rgba(255,255,255,.07)",border:"1px solid rgba(139,92,246,.4)",borderRadius:7,fontSize:12,padding:"8px 12px"}}/>
          </div>
      ...
      <div style={{padding:"14px 14px 60px",maxWidth:520,margin:"0 auto"}}>
            ...
            {ANGLES.map((a,i)=>{
              ...
              return(
                <button key={a.id} onClick={()=>setAngle(i)} style={{padding:"9px 8px",border:angle===i?"2px solid #FFD700":"2px solid rgba(255,255,255,.07)",borderRadius:8,background:angle===i?"rgba(255,215,0,.1)":"rgba(255,255,255,.03)",color:angle===i?"#FFD700":"rgba(255,255,255,.6)",fontSize:11,fontWeight:700,textAlign:"center",lineHeight:1.3}}>
                  <div>{a.emoji}</div><div style={{marginTop:2}}>{a.label}</div>
        ...
        {output&&(
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(0,71,171,.35)",borderRadius:12,padding:16,marginBottom:12}}>
            ...
            <pre style={{fontSize:12,lineHeight:1.75,color:"rgba(255,255,255,.87)",margin:0}}>{output}</pre>
            {isGrokTab&&(
        ...
        {[{text:grokShortOut,label:"⚡ Grok Short",sub:"10-15 sec",ck:"gs",dur:12},{text:grokOut,label:"⚡ Grok Full",sub:"Full prompt",ck:"gf",dur:10}].map(({text,label,sub,ck,dur})=>!text?null:(
          <div key={ck} style={{background:"rgba(76,29,149,.1)",border:"1px solid rgba(139,92,246,.35)",borderRadius:12,padding:16,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div>
                ...
                <div style={{fontSize:10,color:"rgba(255,255,255,.3)",marginTop:2}}>{sub}</div>
              </div>
            ...
            <pre style={{fontSize:12,lineHeight:1.75,color:"rgba(255,255,255,.83)",margin:"0 0 14px"}}>{text}</pre>
            <button onClick={()=>generateVideo(text,dur)} disabled={videoLoading} style={{width:"100%",padding:"11px",background:videoLoading&&activePrompt===text?"rgba(255,215,0,.15)":proxyOk===true?"linear-gradient(135deg,#B45309,#D97706)":"rgba(109,40,217,.3)",color:proxyOk===true?"#fff":"#A78BFA",border:proxyOk===true?"none":"1px solid rgba(139,92,246,.4)",borderRadius:8,fontSize:12,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              {videoLoading&&activePrompt===text?<><span style={{width:12,height:12,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",display:"inline-block",animation:"spin .7s linear infinite"}}/>Generating video...</>:proxyOk===true?"🎬 Generate Video with Grok":"⚡ Setup Proxy to Generate Video"}
            </button>
          </div>
        ))}
        ...
        {(videoLoading||videoStatus)&&(
          <div style={{background:"rgba(0,0,0,.35)",border:"1px solid rgba(255,215,0,.2)",borderRadius:12,padding:16,marginBottom:12}}>
            ...
            {videoStatus==="done"&&videoUrl&&(
              <div>
                ...
                <div style={{display:"flex",gap:8,marginTop:10}}>
                  <a href={videoUrl} download="writershub.mp4" style={{flex:1,textAlign:"center",background:"#00A86B",color:"#fff",padding:"10px",borderRadius:8,fontSize:12,fontWeight:700,display:"block"}}>⬇ Download</a>
            ...
            {!videoLoading&&videoStatus!=="done"&&videoStatus&&<div style={{fontSize:11,color:"rgba(255,100,100,.8)",lineHeight:1.6}}>{videoStatus}</div>}
          </div>
        )}
      </div>
    </div>
